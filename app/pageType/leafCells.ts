...
export async function traverseLeafCellPointer(databaseFileHandler:any,pageOffset:number,columnPosition:Array<number>,whereClause:string,whereClauseColumnPosition:number,whereClauseValue:string,isRowId:boolean){
    ...
    for(let i=0;i<cellPointerArrayLength;i++){
        ...
        consoleRowName(cellOffset,pageView,pageBuffer,columnPosition,databaseFileHandler,whereClause,whereClauseColumnPosition,whereClauseValue,isRowId);
    }
}